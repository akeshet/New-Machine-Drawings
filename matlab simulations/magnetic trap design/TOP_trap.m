...
I =50;                                        %current through coils (A)
coil = [30];                              %coil configuration. from inner set to outer
r = 3.5*inch;                               %Inner coil radius (center of innermost winding)(cm)
zz = 12*inch;                             %Coil separation from center of chamber
z0=12*inch;                     %First coil height
%----------------------------------------------------------------------------
...
for k=1:NN
    ...
    for j=1:coil(k)
        ...
        for i=1:N
            ...
            Bz_d(i,:) = Bz_d(i,:) + B_loopN([0 0 z(i)], -zj, rk, 0);
            By_d(i,:) = By_d(i,:) + B_loopN([0 z(i) 0], -zj, rk, 0);
        end
    end
