function B = BrzAH(r, z0, I, a, coil, Z)
%
...
drz = (a+0.01)*in;                   % extra spacing due to glue + epoxi
for k=1:NN
    rk = r + (k-1)*drz;
    for j=1:coil(k)
        zj = z0 + (j-1)*drz;
...
Gz = mean( gradient(Bz,0.05) );    % average gradient in Z direction
Gr = mean( gradient(By,0.05) );    % average gradient in X,Y directions
...
fprintf('\r\t\tinner D: %1.2fcm\r\t\touter D: %1.2fcm\r\t\tcoil height: %1.2fcm',Di,Do,Dz);
